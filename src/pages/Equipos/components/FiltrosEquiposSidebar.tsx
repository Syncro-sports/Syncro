...
import {
  FiltrosEquipos,
  FILTROS_EQUIPOS_INICIALES,
  NivelEquipo,
  SuperficieEquipo,
  TipoEquipo,
  UBICACIONES_DISPONIBLES,
} from "../equiposData";
import "./FiltrosEquiposSidebar.css";

interface FiltrosEquiposSidebarProps {
  onAplicar: (filtros: FiltrosEquipos) => void;
}

const TIPOS_OPCIONES: TipoEquipo[] = [
  "FUTBOL 5",
  "FUTBOL 7",
  "FUTBOL 8",
  "FUTBOL 9",
...
const SUPERFICIES_OPCIONES: SuperficieEquipo[] = [
  "CESPED SINTETICO",
  "CESPED NATURAL",
  "CEMENTO",
];

const NIVELES_OPCIONES: NivelEquipo[] = ["A", "B", "C"];

const toggleItem = <T,>(list: T[], item: T): T[] => {
  return list.includes(item) ? list.filter((x) => x !== item) : [...list, item];
};

const FiltrosEquiposSidebar = ({ onAplicar }: FiltrosEquiposSidebarProps) => {
  const [tipos, setTipos] = useState<TipoEquipo[]>(FILTROS_EQUIPOS_INICIALES.tipos);
  const [superficies, setSuperficies] = useState<SuperficieEquipo[]>(
    FILTROS_EQUIPOS_INICIALES.superficies
  );
  ...
  return (
    <aside className="filtros">
      ...
      <div className="filtros__group">
        <label htmlFor="filtro-equipos-ubicacion">Ubicación</label>
        <select
          id="filtro-equipos-ubicacion"
          value={ubicacion}
